...
/**
 * Run a faithful in-page WebAuthn ceremony against [beginPath]/[completePath]. The server returns
 * Yubico `{publicKey:…}` options (base64url); we decode them for the WebAuthn API, let the virtual
 * authenticator answer navigator.credentials.create|get, and POST the result. Reads the page's _csrf
 * meta (same as the hosted bundle). `kind`: "create" (register) or "get" (assertion).
 */
function runCeremony(page: Page, kind: "create" | "get", beginPath: string, completePath: string, opts: { timeoutMs?: number } = {}): Promise<CeremonyResult> {
  return page.evaluate(async ({ kind, beginPath, completePath, timeoutMs }) => {
    ...
    const begin = await fetch(beginPath, { method: "POST", headers: { "Content-Type": "application/json", [hdr]: meta } });
    ...
    if (kind === "create") {
      ...
      const complete = await fetch(completePath, { method: "POST", headers: { "Content-Type": "application/json", [hdr]: meta }, body: JSON.stringify(payload) });
  ...
  }, { kind, beginPath, completePath, timeoutMs: opts.timeoutMs });
}
...
/** Fill + submit the hosted password form; return the identity origin it is served from. */
  ...
  test("full journey: password sign-in → enrol passkey → enforce → re-sign-in is passkey-challenged → /protected", async ({
    ...
    try {
      ...
      await test.step(STEPS.enroll, async () => {
        await page.goto(`${identityOrigin}/passkeys/register`, { waitUntil: "domcontentloaded" });
